...
- **`sf` runner** (`@rules/sf-cli.md`): every `sf` call via `src/sf/runner.ts` using **`cross-spawn`** with an args array (`--json` appended) — never `node:child_process`, never a concatenated shell string, never a spawn from `commands`/`services`. Binary resolved from `SF_CLI_PATH` / `sfPath` else `PATH` (cross-OS, no platform branch); `ENOENT` → a clear "sf not found" error. `sf` v2 only, never `sfdx`. Large text args (SOQL/SOSL) go through `runWithLargeArg` (temp `--file` / `--query-file` when they would overflow the Windows ~8191-char command line); `run()` guards the length and fails clearly, never the opaque "Réponse sf illisible".
...
- **Runtime feedback** (`@rules/progress.md`): every tool ships `src/progress.ts`; services wrap each `sf` operation with `progress.step` (a count on success), commands bracket the file write. The reporter writes **only** to `stderr`, auto-on a TTY, `--no-progress` disables it; `cli.ts` raises the `pino` level to `warn` while it is active. `output/` never imports `progress`; no `console.*`; no new dependency.
...
- **`CLAUDE.md`** at the generated project root (in the user's language), recording the tool's identity for future sessions:
  ...
  ```
  `[nom-outil]` = the tool name (`APP_NAME`). The version here is the **framework** version declared at the top of the framework `CLAUDE.md` (currently 1.2.0) — not the tool's own version (which starts at 1.0.0 in `package.json` / `docs/release/CHANGELOG.md`). Replace the `Deviations` list with every deviation validated via the Phase 4/5 deviation protocol (`- [deviation] — reason: [justification]`); if none, keep `- None`.
- **`.claude/settings.json`** at the generated project root so the tool stays self-enforced in later maintenance sessions:
  ...
  ```json
  {
    "permissions": {
      ...
      "deny": ["Read(**/.env)", "Read(**/.env.*)", "Read(**/secrets/**)", "Write(**/.env)", "Write(**/.env.*)", "Write(**/secrets/**)", "Edit(**/.env)", "Edit(**/.env.*)", "Edit(**/secrets/**)", "Write(**/node_modules/**)", "Write(**/dist/**)"]
    },
    "hooks": {
