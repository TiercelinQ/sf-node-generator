...
Detailed in `@rules/verification.md`. Key points: all `sf` / external-CLI spawns go through `src/sf/runner.ts` via **`cross-spawn`** with an **args array** (no `node:child_process` direct, no shell, no spawn from `commands`/`services`/`output`); user values are discrete array elements; external data (`sf --json`, `sfdx-project.json`, args, files, env) is validated from `unknown`; any path built from input is resolved and confined under the base dir (no traversal); **no secret** in `.env` / `config.ts` / logs / `Result` messages / commits (`sf` keychain, non-secret alias only); `.env` and the export dir gitignored; no `eval` / `new Function` / `vm` on untrusted input; `npm audit` in the last-batch instructions; the README documents the `sf` prerequisite + `SF_CLI_PATH`.
