...
Detailed in `@rules/verification.md`. Key points: `src/logger.ts` present and conforming (pino → **file + `stderr`**, `pino-pretty` dev-only, level from `config.logLevel` with `[TOOL]_DEBUG` → debug); `log` exported as the single instance; `configureLogger()` called first in `cli.ts` (before argv dispatch); `pino` **and** `pino-pretty` both in `dependencies` (pino-pretty is imported at runtime by the logger, so it must not be a devDependency — `@rules/config.md`); **no `console.log` / `console.error`** in delivered `src/`; `stdout` never receives a log line (data-only); every non-re-throwing `catch` calls `log.error({ err }, …)`; the global `uncaughtException` / `unhandledRejection` handlers log via pino then exit 1; no token/secret/PII in any log call; `LOG_FILE` under `logs/` (gitignored).
